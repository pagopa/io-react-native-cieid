...
///  Edited by hevelius on 23/05/2024
///     - removing loading of url and url scheme from Info.plist
///     - adding sp_url and sp_url_scheme as parameters
...
class CieIDWKWebViewController: UIViewController, WKNavigationDelegate {
  ...
  override func loadView() {
    ...
    if #available(iOS 13, *) {
      ...
      if let spUrlString : String = sp_url {
        ...
        print("CieID SDK ERROR: Service provider URL non presente in Info.plist 1")
  ...
  internal func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: ((WKNavigationActionPolicy) -> Void)) {
    ...
    if let urlCaught = navigationAction.request.url {
      ...
      if (urlCaught.absoluteString.containsValidIdpUrl){
        ...
        if let urlToCieID = urlCaught.appendSourceAppParameter(sp_url_scheme: sp_url_scheme!) {
          ...
          print("CieID SDK ERROR: Service provider URL Scheme non presente in Info.plist 2")
